...
.topbar{position:relative;z-index:10;display:flex;align-items:center;justify-content:space-between;padding:16px 20px 10px;}
...
.viz-wrap{position:relative;z-index:10;height:80px;display:flex;align-items:flex-end;justify-content:center;gap:3px;padding:0 24px;margin-bottom:4px;}
.viz-bar{flex:1;max-width:10px;border-radius:3px 3px 0 0;background:linear-gradient(to top, var(--netlify), var(--netlify-bright));box-shadow:0 0 6px var(--netlify);transition:height 0.08s ease;transform-origin:bottom;}

.art-section{position:relative;z-index:10;display:flex;justify-content:center;padding:0 32px;margin-bottom:16px;}
.art-outer{width:200px;height:200px;border-radius:50%;padding:3px;background:conic-gradient(from 0deg, var(--netlify), var(--netlify-bright), var(--netlify-glow), var(--netlify-dim), var(--netlify));box-shadow:0 0 40px rgba(0,200,83,0.4), 0 0 80px rgba(0,200,83,0.15), inset 0 0 20px rgba(0,0,0,0.5);animation:spin-ring 8s linear infinite;animation-play-state:paused;}
.art-outer.playing{animation-play-state:running;}
@keyframes spin-ring{to{transform:rotate(360deg);}}
.art-inner{width:100%;height:100%;border-radius:50%;background:var(--surface);display:flex;align-items:center;justify-content:center;overflow:hidden;border:2px solid var(--bg);}
.art-inner img{width:100%;height:100%;object-fit:cover;border-radius:50%;}
.art-placeholder{display:flex;flex-direction:column;align-items:center;gap:6px;}
...
.art-hole{position:absolute;width:20px;height:20px;border-radius:50%;background:var(--bg);border:2px solid var(--border-bright);box-shadow:0 0 8px var(--netlify);z-index:2;}

.track-info{position:relative;z-index:10;text-align:center;padding:0 28px;margin-bottom:14px;}
.track-name{font-family:'Orbitron',monospace;font-size:17px;font-weight:700;letter-spacing:2px;color:var(--netlify-bright);text-shadow:0 0 16px rgba(0,230,118,0.6);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-bottom:4px;}
...
.progress-wrap{position:relative;z-index:10;padding:0 24px;margin-bottom:10px;}
...
.progress-track{height:4px;background:var(--surface2);border-radius:2px;cursor:pointer;position:relative;overflow:visible;}
.progress-fill{height:100%;border-radius:2px;background:linear-gradient(90deg, var(--netlify-dim), var(--netlify-bright));box-shadow:0 0 10px var(--netlify);position:relative;transition:width 0.3s linear;}
.progress-fill::after{content:'';position:absolute;right:-5px;top:50%;transform:translateY(-50%);width:10px;height:10px;border-radius:50%;background:var(--netlify-glow);box-shadow:0 0 12px var(--netlify-bright), 0 0 24px var(--netlify);}

.controls{position:relative;z-index:10;display:flex;align-items:center;justify-content:center;gap:20px;padding:4px 24px 10px;}
.ctrl-btn{background:transparent;border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;color:var(--muted);transition:all 0.2s;padding:8px;border-radius:50%;-webkit-tap-highlight-color:transparent;}
...
.play-btn{width:68px;height:68px;border-radius:50%;background:linear-gradient(135deg, var(--netlify), var(--netlify-bright));border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;box-shadow:0 0 30px rgba(0,200,83,0.6), 0 0 60px rgba(0,200,83,0.2);transition:all 0.15s;color:#001a0a;-webkit-tap-highlight-color:transparent;position:relative;}
.play-btn::before{content:'';position:absolute;inset:-3px;border-radius:50%;background:conic-gradient(var(--netlify-glow), transparent 60%, var(--netlify-glow));animation:play-ring 2s linear infinite;animation-play-state:paused;opacity:0;transition:opacity 0.3s;}
...
.volume-row{position:relative;z-index:10;display:flex;align-items:center;gap:10px;padding:0 24px 6px;}
...
.bottom-tabs{position:relative;z-index:10;display:flex;border-top:1px solid var(--border);background:rgba(4,18,16,0.8);backdrop-filter:blur(10px);}
...
.panel{position:relative;z-index:10;flex:1;overflow-y:auto;padding:0 16px 8px;display:none;}
...
.search-bar{display:flex;align-items:center;gap:8px;background:var(--surface2);border:1px solid var(--border);border-radius:10px;padding:8px 12px;margin-bottom:12px;}
.search-bar input{flex:1;background:transparent;border:none;outline:none;font-family:'Rajdhani',sans-serif;font-size:14px;color:var(--text);letter-spacing:0.5px;}
.search-bar input::placeholder{color:var(--muted);}
.search-bar svg{width:16px;height:16px;color:var(--muted);flex-shrink:0;}

.track-row{display:flex;align-items:center;gap:12px;padding:10px 12px;border-radius:10px;cursor:pointer;transition:all 0.15s;border:1px solid transparent;margin-bottom:4px;}
...
.upload-zone{border:2px dashed var(--border-bright);border-radius:14px;padding:24px;text-align:center;cursor:pointer;transition:all 0.2s;margin-bottom:14px;background:rgba(0,200,83,0.02);}
...
.toast{position:fixed;bottom:80px;left:50%;transform:translateX(-50%) translateY(20px);background:var(--surface2);border:1px solid var(--border-bright);border-radius:20px;padding:10px 20px;font-family:'Orbitron',monospace;font-size:12px;color:var(--netlify-bright);box-shadow:0 0 20px rgba(0,200,83,0.3);z-index:500;opacity:0;transition:all 0.3s;pointer-events:none;white-space:nowrap;}
...
const Icon = {
  ...
  Heart: ({filled}: {filled: boolean}) => <svg viewBox="0 0 24 24" fill={filled?"currentColor":"none"} stroke="currentColor" strokeWidth="2"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>,
};
...
export default function B2Player() {
  ...
  const loadFiles = (files: FileList | File[]) => {
    ...
    setTracks(prev => {
      ...
      if (wasEmpty) setTimeout(() => setPlaying(true), 100);
    ...
    <>
      ...
      <div className="app">
        ...
        {tab === "player" && (
          <>
            ...
            <div className="volume-row">
              <button className="ctrl-btn sm" onClick={() => setMuted(m => !m)}>
        ...
        <div className="bottom-tabs">
          <button className={`btab ${tab === "player" ? "active" : ""}`} onClick={() => setTab("player")}>
            <Icon.Music /> PLAYER <div className="btab-line" />
          </button>
          <button className={`btab ${tab === "library" ? "active" : ""}`} onClick={() => setTab("library")}>
            <Icon.List /> LIBRARY <div className="btab-line" />
          </button>
        </div>
